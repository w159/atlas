...
| `orc-planner` | decompose a task into a numbered, failable-check stage map | sonnet | no |
| `orc-docs-curator` | keep `docs/` as the single source of truth, current with the work | sonnet | only under `docs/` |
| `orc-docs-auditor` | audit `docs/` for drift against the code/behavior | sonnet | no |
| `orc-completeness-critic` | final "what did we miss" gap pass; findings seed the next wave | sonnet | no |
...
- ❌ Returning raw logs/diffs in the final message: return the distilled report; write bulky evidence to `docs/evidence/`.
