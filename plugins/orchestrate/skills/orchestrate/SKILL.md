...
- **Your own `Write`/`Edit` is confined to orchestration artifacts:** `docs/.run/` state (STATE.md, findings.json, work-log.md), `docs/plans/` stage maps, `docs/evidence/` captures, and the project's living `docs/` tree generally. Nothing else. (Durable docs/ edits are normally delegated to `orc-docs-curator`; see below.)
...
`orc-docs-curator` is the only writer of durable `docs/` content (it writes nowhere else); `orc-docs-auditor` reads the tree against the code and reports drift. Keeping docs/ current is **gate-enforced** at completion, not optional cleanup. Taxonomy and templates: `references/docs-ssot.md`. Per-root placement: `references/scaffolding.md`.
...
- **Sharpen prompts before spending tokens.** Optimize the user's incoming prompt (shipped `UserPromptSubmit` hook) and every outbound subagent spec. The `/orchestrate:orc-prompt` command rewrites a vague request into an environment-aware, verification-gated spec before any dispatch. See `references/prompt-optimization.md`.
...
- **docs/ is current** - `CHANGELOG.md` and `ROADMAP.md` reconciled, and every affected durable subfolder (`architecture/`, `features/`, `specs/`, `audits/`, `lessons/`, etc.) updated by `orc-docs-curator`. This is mandatory and gate-enforced, not optional cleanup.
...
| **sonnet** | implementation, most subagent work, running & writing tests, DB probing, docs curation | default; `orc-implementer`, `orc-db-prober`, `orc-docs-curator` |
| **opus** | hard architecture, security reasoning, multi-stage decomposition, cross-validation of critical findings, completeness critique, final synthesis | you; `orc-planner`, `orc-completeness-critic`, `Agent(model:"opus")` on `orc-verifier` for critical items |
...
- **Orchestrator companions** (carry this skill's discipline): `orc-explorer` (read-only mapping), `orc-implementer` (one bounded change), `orc-verifier` (adversarial confirmation), `orc-db-prober` (read-only DB), `orc-ui-runtime-tester` (live frontend behavior), `orc-planner` (multi-stage decomposition + stage maps), `orc-docs-curator` (maintains docs/ SSOT; writes only under `docs/`), `orc-docs-auditor` (audits docs/ for drift against code), `orc-completeness-critic` ("what did we miss" gap pass before done).
- **UI/UX test swarm** (project-independent, browser-driven, detect+report only): `orc-ux-cartographer` (discover routes/fields + the live save/read-back contract), `orc-ux-persona` (generate a persona, enroll, enter data, walk the UI, file bugs/user-stories/feedback/feature-requests), `orc-ux-fuzzer` (boundary/fuzz the discovered inputs), `orc-ux-accuracy-oracle` (independently recompute every client-facing number), `orc-ux-reporter` (synthesis + the three hard gates + deliverables). The full runbook is `references/ux-test-swarm.md`.
...
This skill ships as part of the **orchestrate plugin**: the fourteen `orc-*` companions live in the plugin's top-level `agents/` directory (`plugins/orchestrate/agents/`) and are auto-discovered by Claude Code; the automation hooks under `hooks/` ship with their installer.
