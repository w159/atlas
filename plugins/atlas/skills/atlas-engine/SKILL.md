...
0. **Orient (you, cheap).** Detect project + codebase roots (dirs with their own manifest). Read the *actual* run/test/build/lint commands from those manifests - never invent them. Query `claude-mem`/`ctx_search` for prior work. Note live `serena`/LSP/MCP/skill capabilities. Scaffold per-root `docs/` artifacts. On boot or resume, reconcile recent work against docs/ per `references/session-lifecycle.md` before planning new work. Present a 5-10 line orientation + plan. **Gate before mutating anything.**
