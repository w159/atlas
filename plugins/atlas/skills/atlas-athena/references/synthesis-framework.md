...
How atlas-athena turns seven independent dimension reviews into one
prioritized, file:line-anchored report. Read this when you are running
Phase 4 (synthesize and output) of an athena run, or when a report came
...
to atlas-ariadne. A reviewer that surfaces it notes the finding as
...
The hub is built after handoffs, per `atlas-ariadne`'s
`references/graph-to-hub-pipeline.md`. The same `build_hub.py` script
produces `hub/manifest.json` and `hub/index.html` so athena findings are
...
is satisfied, the same completion gate atlas-metis enforces.
...
- Mixing in a structural-duplication finding. That is ariadne's scope.
