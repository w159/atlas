...
This file makes the .atlas/docs/ lifecycle explicit at the two session boundaries. It does
not redefine the .atlas/docs/ layout or the "move done items" convention - those live in
...
2. **Diff recall against docs/.** Compare that recalled reality to `.atlas/docs/CHANGELOG.md`,
   `.atlas/docs/ROADMAP.md`, `.atlas/docs/AGENTS.md`, and the durable subfolders. Look for docs that
...
against `git log` plus the .atlas/docs/ tree alone. The diff-and-correct steps are unchanged;
...
For that in-flight work the orchestrator maintains a durable run-state under `.atlas/docs/.run/`
...
1. **The durable run-state the orchestrator keeps.** While a multi-wave task is in flight,
   ...
   - `.atlas/docs/.run/STATE.md` - the live map of the run: the current wave, the stages already
   ...
   - `.atlas/docs/.run/findings.json` - the authoritative per-stage findings and verdicts (schema
   ...
   - `.atlas/docs/.run/work-log.md` - an append-only log of what each wave dispatched, what it
...
2. **The resume protocol on boot or resume.** When a session opens onto an in-flight run:
   - **Read the run-state first.** Re-read `.atlas/docs/.run/work-log.md`, then `STATE.md`, then
   ...
   - **Reconcile it against reality.** Confirm the run-state matches the actual repo and
     .atlas/docs/ state: a stage STATE.md calls done must be backed by a `verified` entry in
...
3. **Archive on completion so history persists.** When the run finishes, copy the final
   ...
   survive after `.atlas/docs/.run/` is cleared for the next task. The durable narrative still
...
**Durable vs ephemeral - the intended reading.** `docs-ssot.md` classifies `.atlas/docs/.run/` as
the one ephemeral, gitignored subtree, and that holds: nothing in `.atlas/docs/.run/` is committed.
...
makes resume possible. Completion resolves the two: `.atlas/docs/.run/` is cleared (ephemeral as
...
`.atlas/docs/evidence/` at the moment of proof, as the rest of the skill already requires.
...
This is the same docs-current condition the atlas-metis completion gate already enforces;
