...
/graphify <path> --exclude <glob>                     # skip paths (gitignore-style glob, repeatable) - scopes large repos
...
result = detect(Path('INPUT_PATH'), extra_excludes=EXCLUDES)
print(json.dumps(result))
" > .graphify_detect.json
```

Replace INPUT_PATH with the actual path the user provided, and EXCLUDES with a Python list of
any `--exclude <glob>` patterns the user passed (default `[]` when none). Do NOT cat or print the
JSON - read it silently and present a clean summary instead:
...
- If `total_words` > 2,000,000 OR `total_files` > 200 (the size gate), branch on whether this is
  an interactive run:
  ...
  - **Non-interactive** (`GRAPHIFY_NONINTERACTIVE=1` is the authoritative signal; a non-TTY stdin
    ...
    (`re-invoke on a narrower path or add --exclude <glob> to trim it`). Never block on input.
- Otherwise: proceed directly to Step 2.5 if video files were detected, or Step 3 if not.

#### Scoping a large repo (how `detect` decides what to read)

`detect()` already trims most noise, so reach for these before assuming a repo is "too big":

- **Built-in pruning** - `detect`'s `_SKIP_DIRS` skips `node_modules`, `.venv`/`venv`, `dist`,
  `build`, `target`, `__pycache__`, `.ruff_cache`, `.next`/`.nuxt`/`.turbo`, `graphify-out`,
  `.worktrees`, and similar generated/vendor dirs automatically. You do not need to exclude these.
- **`.graphifyignore` / `.gitignore`** - `detect` reads a `.graphifyignore` at (and above) the
  scan root, gitignore syntax; if absent it falls back to the repo's `.gitignore`. Use this to
  carve out repo-specific generated trees (bundled archives, audit outputs) without retyping them
  each run.
- **`--exclude <glob>`** - per-invocation patterns, repeatable; anchored at the scan root and
  applied last, so they win over the ignore files. Forwarded into `detect(..., extra_excludes=[...])`.
