...
This reference documents the Phase 4 pipeline that turns ariadne's per-root
...
atlas-ariadne run, or debugging a hub that came out wrong.
...
- One `graph.json` per discovered root, written by the `graphify` skill under
  each root's `graphify-out/`. Ariadne runs graphify per root (Phase 1) so each
...
| `kind` | string | always `"finding"` for ariadne handoffs |
...
## What ariadne is responsible for

Ariadne owns the hub only for structural-duplication findings: the
...
atlas-athena and must not appear in an ariadne hub. If a handoff file
...
keep ariadne handoffs purely structural.
