...
| Full UI/UX test pass / persona testing / pre-release UX sweep (any app) | (orchestrator dispatches atlas-odysseus) | `atlas-odysseus` (canonical home; auto-discovers routes and fields) | Chrome DevTools MCP / Claude_Preview MCP / `browser-harness` / playwright, `context-mode` | sonnet; opus for the reporter |
...
| Comprehensive quality + security + OWASP audit (full codebase sweep) | (orchestrator dispatches atlas-athena) | `atlas-athena` | `serena`, `context7`, `context-mode`; no browser needed | sonnet (multi-swarm) |
| Architecture map / structural dedup / boundaries doc missing | (orchestrator dispatches atlas-ariadne) | `atlas-ariadne` | `serena`, LSP, `context-mode` | sonnet |
| Recurring or iterative task / needs a reusable loop | (orchestrator dispatches atlas-chronos) | `atlas-chronos` | - | sonnet |
| Vendor MCP connector setup / credentials to wire | (orchestrator dispatches atlas-hermes) | `atlas-hermes` | - | sonnet |
| Measure run health / self-improvement from observability data | (orchestrator dispatches atlas-argus) | `atlas-argus` | SQLite observability DB (`~/.atlas/atlas.db`) | sonnet |
| Project boot / onboarding / configure tooling for a repo | (orchestrator dispatches atlas-hephaestus) | `atlas-hephaestus` | `serena`, hook/config wiring | sonnet |
| Multi-stage / multi-surface orchestration (build/fix/audit/refactor spanning several subagents) | (this skill is the orchestrator itself - normally entered via its own skill trigger, not routed to from within a session) | `atlas-metis` | whatever Step 1 discovers live | sonnet |
| Choosing / stacking which skills to run for a goal or stack | (orchestrator dispatches atlas-nestor) | `atlas-nestor` | - | sonnet |
