...
when_to_use: Running a weekly, monthly, or quarterly metrics review; investigating a sudden metric spike or drop; comparing performance against targets or the prior period; turning raw numbers into a scorecard with recommended actions.
...
Structure the review using a metrics hierarchy: North Star metric at the top, L1 health indicators (acquisition, activation, engagement, retention, revenue, satisfaction), and L2 diagnostic metrics for drill-down. The full hierarchy definitions, common metric definitions (DAU/WAU/MAU, retention, conversion, activation), goal-setting frameworks (OKRs), review cadences, and dashboard design principles live in [references/metrics-frameworks.md](references/metrics-frameworks.md).
