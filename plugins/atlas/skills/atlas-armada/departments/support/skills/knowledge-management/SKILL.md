...
when_to_use: A ticket is resolved and the solution should be documented for reuse; an existing KB article needs updating after a product change; you are creating a how-to, troubleshooting, or FAQ article from scratch.
...
Four article types cover most KB content: how-to, troubleshooting, FAQ, and known issue. Each has a distinct structure. See `references/article-templates.md` for the full template and best practices for each type.

- **How-to**: step-by-step instructions for a task (Prerequisites, Steps, Verify, Common Issues)
- **Troubleshooting**: diagnose a problem (Symptoms, Cause, Solution options, Prevention)
- **FAQ**: quick answer to a common question (Direct answer, Details, Related Questions)
- **Known issue**: documented bug or limitation (Status, Symptoms, Workaround, Fix Timeline, Updates)
