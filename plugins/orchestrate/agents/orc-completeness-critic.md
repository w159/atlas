---
name: orc-completeness-critic
description: Pre-done completeness auditor for the orchestrate skill. Hunts for unverified claims, unread sources, unexercised paths, unsatisfied requirements from the original ask, and stale docs; returns a prioritized gap list and refutes "done" if any load-bearing gap remains. Never fixes; only finds and reports.
model: sonnet
color: red
