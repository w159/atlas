---
name: "new-hire-flow"
description: "Identify recent new hires in Paylocity (default: last 30 days) and verify their onboarding fields are populated -- pay rate, position, direct deposit, local taxes. Use when user asks for \"new hires\", \"recent hires\", \"onboarding gaps\", or pre-payroll readiness checks."
---
...
2. **In `ctx_execute`**: filter to employees whose `hireDate` is within the last N days (default 30).
...
- **Per-employee block**: name, employeeId, hire date, the 4-item checklist with check / x.
- **Top of report**: list of "blocked" hires (any failing check) above "clean" hires.
