...
function classifyError(operation: string, err: unknown): Classification {
  ...
  // (.status: CwHttpError / node-fetch / undici; .statusCode: CIPP service errors)
...
function isStatusError(
  ...
  const e = err as Record<string, unknown>;
  // Accept either .status (CwHttpError / node-fetch / undici) or .statusCode (CIPP service errors)
